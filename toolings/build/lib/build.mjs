...
export async function runBuild({ root, name, modules, server, outDir }, { watch = false } = {}) {
  ...
  // bundles contain only the app's own code. Chunk filenames carry a content
  // hash; entry URLs are cache-busted by the manifest version instead.
  const clientConfig = {
  ...
  function checkServerExports(serverMeta) {
    const [serverOut] = Object.values(serverMeta.outputs)
  ...
  // per moduleId, with a content-hash version over ALL artifacts (entries
